...
// 原型对象 添加click方法
Init.prototype.click = function(callback){// 作用域1
    // 循环标签数组
    for(var index = 0 ; index < this.elements.length ; index ++){
        //事件监听
        this.elements[index].addEventListener("click",function(event){// 作用域2
            //判断是否存在callback回调函数，存在则调用回调函数
            callback && callback(event);// // 作用域3
        })
    }
}    

// 设置myJquery别名
window.$ = window._ = window.myJquery = myJquery;
